...
<div class="form-card">
    <header style="text-align: center; margin-bottom: 3rem;">
        ...
        <h2 style="font-size: 1.75rem; font-weight: 800; color: var(--text-main); margin-bottom: 0.5rem;">New Category</h2>
        <p style="color: var(--text-muted); font-size: 0.95rem;">Please fill in the details below to create a new category.</p>
    </header>
    ...
    <form method="POST" action="{{ route('admin.categories.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="e.g., Premium Cashews" required autofocus>
            @error('name')
        ...
        <div class="form-group">
            ...
            <textarea id="description" name="description" placeholder="Provide a brief description of this category...">{{ old('description') }}</textarea>
            @error('description')
            <div style="color: var(--danger); font-size: 0.85rem; margin-top: 0.5rem; display: flex; align-items: center; gap: 0.35rem;">
                <i class="fas fa-circle-exclamation"></i> {{ $message }}
            </div>
